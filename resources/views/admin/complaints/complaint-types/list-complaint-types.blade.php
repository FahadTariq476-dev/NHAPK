            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                            <h2 class="content-header-title float-start mb-0">List Complaint Types</h2>
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">List Complaint Types</li>
                                </ol>
                            </div>
                        </div>
                    </div>
            ...
            <div class="content-body">
                <div class="row">
                    <div class="col-12">
                        ...
                        @endif
                            <table class="table mb-0 dataTable" id="complaint_typesTable" style="background-color: #f2f2f2; color: #333;">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>Title</th>
   ...
   {{-- End: Modal --}}
    ...
    <script>
        ...
        jq(document).ready(function() {
            var dataTable = jq('#complaint_typesTable').DataTable({
                columns: [
                    { data: 'id' },
                    { data: 'name' },
                    {
                    ...
                    { 
                        ...
                        render: function(data, type, row) {
                            ...
                            var statusText = data == '0' ? 'In-Active' : 'Active';
                            var statusClass = data == '0' ? 'badge bg-warning' : 'badge bg-success';
                    ...
                    { 
                        data: null,
                        render: function(data, type, row) {
                            var statusSelect = '<select class="status-select form-select form-control form-select-sm" data-id="' + row.id + '">';
                            statusSelect += '<option value="" disabled>Select Status</option>';
                            statusSelect += '<option value="1" ' + (row.status == '1' ? 'selected' : '') + '>Active</option>';
                            statusSelect += '<option value="0" ' + (row.status == '0' ? 'selected' : '') + '>In-active</option>';
                            statusSelect += '</select>';
                            return statusSelect;
                        }
                    },
                ...
                ajax: {
                    url: "/admin/complaint-types/list-complaint-types/get-complaint-types",
                    type: "GET",
                    data: function(d) {
        ...
        jq('#complaint_typesTable')
            .on('change', '.status-select', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        jq.ajax({
                            url: '/admin/complaint_types/list-complaint-types/update-status/' + selectedValue + '/' + dataId,
                            type: 'get',
            ...
            .on('click', '.edit-btn', function() {
                ...
                Swal.fire({
                    title: "Are you sure?",
                    text: "You want to edit the complaint types of: " + dataId + "?",
                    icon: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#3085d6",
                    cancelButtonColor: "#d33",
                    ...
                    if (result.isConfirmed) {
                        // Proceed with your action here
                        window.location.href = '/admin/complaint-types/edit-complaint-types/' + dataId;
                    } else {
                        Swal.fire("Cancelled", "You pressed Cancel!", "info");
                    }
                });
            ...
            .on('click', '.show-btn', function() {
                ...
                // Fetch the complaint_types by description
                jq.ajax({
                    url: '/admin/complaint-types/list-complaint-types/description/' + dataId, 
                    method: 'GET',
                    success: function(response) {
