...
class NewsFeedController extends Controller
{
    ...
    public function index(){
        ...
        return view('frontEnd.newsfeed.newsfeeds-view')->with(['news'=> $news]);
    }
    ...
    public function viewFullNewsfeedBySlug($slug){
        ...
        if(count($news)>0){
            return view('frontEnd.newsfeed.newsfeeds-view-detailed')->with(['news'=> $news]);
        }
        else{
            return redirect()->route('frontEnd.newsfeeds-view')->with(['error','You are trying to access invalid news. Kindly try the valid']);
        }
    }
}
